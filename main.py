...
# Page 184
axis1dir = b'\x3f\x10\xf6\x0c\x00\x08\x10\x3a\x98\x00' \
           b'\x00\x00\x0a\x00\x00\x00\x32\x00\x1e\x00' \
           b'\x00\x00\x11'
...
# Page 168
pausecmd = b"\x3f\x06\xf6\x0b\x00\x14"
pausecmd = calc_crc(pausecmd)

print ("Pause command: %s" % pausecmd.encode().hex().upper())

# Page 184
axis0dir = b'\x3f\x10\xf6\x08\x00\x08\x10\x3a\x98\x00' \
            b'\x00\x00\x0a\x00\x00\x00\x32\x00\x1e\x00' \
            b'\x00\x00\x11'
...
class LinearActuator(object):
    ...
    def receiveMultiRead(self, msg, exampleresp=None):
        ...
        numreg = msg[-4:-2].encode('ascii')
        print(numreg)
        #numreg = unpack('<10sHHb', numreg)
        print(numreg)
        numreg = self.twobytefmt.unpack(numreg)[0]
        numbytes = numreg * 2
    ...
    def readStatus(self):
        ...
        if self.simulate:
            resp = b"\x3F\x03\x02\x70\x13\xF5\x8C"
        else:
            resp = None
            self.com.write(msg)
    ...
    def readCurrentPos(self):
        ...
        if self.simulate:
            resp = b"\x3F\x03\x04\x38\xA5\x00\x00\x38\xB3"
            payload = self.receiveMultiRead(msg,exampleresp=resp)
        else:
            self.com.write(msg)
    ...
    def readGatewayStatus(self):
        ...
        msg = hdr + numreg
        msg = calc_crc(msg)
        exresp = b"\x3F\x03\x04\x80\x21\x00\x03\x1C\x3B"
...
class IAI(object):
    def __init__(self, simulate=True):
        self.sim = simulate
        if not simulate:
            self.s = serial.Serial("COM4",baudrate=38400,timeout=0.5)
    ...
    def move0(self, posmm):
        pos = [0,0,0,0]
        axis0setpos = b"\x3f\x10\xf6\x08\x00\x02\x04"
        pos[1] = posmm & 0xFF
    ...
    def pos0(self):
        ...
        if not self.sim:
            ...
            v = b"\x3f\x03\x04\x38\xa5\x00\x00\x38\xb3"
        vnochk = v[:-2]
        vck = calc_crc(vnochk)
        if vck == v:
            print ("crc check ok")
